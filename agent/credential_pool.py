...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    def _get_env_value(key: str, *, prefer_dotenv: bool = False) -> str:
        env_file = load_env()
        if prefer_dotenv:
            # OpenRouter users commonly rotate a key by editing Hermes' .env
            # while an old shell export remains in the parent environment.
            # Prefer the persisted Hermes value for that path, but still fall
            # back to runtime env injection when .env is empty.
