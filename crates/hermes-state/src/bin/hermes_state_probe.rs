...
//! Rust `SessionStore` through a subprocess boundary before the production
//! `SessionDB` swaps implementations.

use hermes_state::{
    AppendMessage, ExportedSession, NewSession, SearchContextMessage, SearchMatch, SearchOptions,
    SessionListOptions, SessionListRecord, SessionRecord, SessionRichOptions, SessionRichRecord,
    SessionStore, StoredMessage, TokenUpdate,
};
...
fn run_operation(store: &mut SessionStore, operation: Value) -> Result<Value, String> {
    let op = operation
        .get("op")
...
fn conversation_message_to_json(message: hermes_state::ConversationMessage) -> Value {
