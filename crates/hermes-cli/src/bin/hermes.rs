...
use hermes_cli::launcher::{
    ...
    is_rust_config_path_request, is_rust_config_set_request, is_rust_config_show_request,
    is_rust_cron_lifecycle_request, is_rust_cron_list_request, is_rust_cron_status_request,
    is_rust_gateway_status_request, is_rust_gateway_stop_request, is_rust_help_request,
    is_rust_logs_request, is_rust_plugins_request, is_rust_profile_request, is_rust_skills_request,
...
use hermes_cli::{
    ...
    run_auth_command, run_config_set_command, run_config_show_command, run_cron_lifecycle_command,
    run_gateway_stop_command, run_logs_command, run_plugins_command, run_skills_command,
...
fn run_rust(args: &[OsString]) -> i32 {
    ...
    if is_rust_auth_request(args) {
        ...
        eprint!("{}", outcome.error);
        return outcome.exit_code;
    }

    if is_rust_config_show_request(args) {
        let outcome = run_config_show_command(args, &profile_context.hermes_home);
        print!("{}", outcome.output);
        return outcome.exit_code;
    }
    ...
    if is_rust_config_set_request(args) {
        let outcome = run_config_set_command(args, &profile_context.hermes_home);
        print!("{}", outcome.output);
        return outcome.exit_code;
    }
    ...
    if is_rust_cron_list_request(args) {
        let mut include_disabled = false;
        for arg in args.iter().skip(2) {
            if arg == OsStr::new("--all") {
                include_disabled = true;
            } else {
                eprintln!("usage: hermes cron list [--all]");
                return 2;
            }
        }
        let list = cron_list(&profile_context.hermes_home, include_disabled);
        print!("{}", render_cron_list(&list));
        return 0;
    }

    if is_rust_cron_lifecycle_request(args) {
        let outcome = run_cron_lifecycle_command(args, &profile_context.hermes_home);
        print!("{}", outcome.output);
        return outcome.exit_code;
    }
    ...
    if is_rust_gateway_stop_request(args) {
        let (output, exit_code) = run_gateway_stop_command(&profile_context.hermes_home);
        print!("{output}");
        return exit_code;
    }
    ...
    if is_rust_plugins_request(args) {
        let outcome = run_plugins_command(args, &profile_context.hermes_home);
        print!("{}", outcome.output);
        return outcome.exit_code;
    }

    if is_rust_skills_request(args) {
        let outcome = run_skills_command(args, &profile_context.hermes_home);
        print!("{}", outcome.output);
        return outcome.exit_code;
    }
