...
| | _Started the full handler cutover gate by adding native Rust todo semantics, clarify validation/result shaping, memory add/replace/remove/threat-scan/snapshot semantics, and an executable coverage check over all 45 core tools from the Rust registry snapshot. Current state remains not deletion-safe; terminal/process/execute_code, browser/web, delegate/subagent, MCP, session_search, media, skills, clarify UI callbacks, cron/messaging/homeassistant, and kanban are explicit deletion blockers with required Rust or external-service migration plans in docs/rust-parity/tool-handler-boundaries.md._ |  |  |  |  |
