...
| | _Started production integration cutover with Rust-owned `hermes cron status`, `hermes cron list [--all]`, `hermes cron pause`, and `hermes cron remove/rm/delete`, matching Python for no-gateway status, active-job counting, disabled-job exclusion, next-run display, empty-list messaging, job table rendering, disabled-job inclusion, skill normalization, repeat/delivery/script/workdir/last-run fields, gateway-not-running warnings, pause state mutation, remove mutation, missing-job messages, and Python's current zero exit status for cron lifecycle failures from cron/jobs.json. Remaining TUI gateway, dashboard backend, ACP, cron create/edit/resume/run/tick scheduler execution, batch, MCP, RL, real session/state/tool/provider integration, and delivery runtimes remain Python-owned._ |  |  |  |  |
