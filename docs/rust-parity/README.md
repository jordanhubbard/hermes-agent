...
| | _Started the production CLI cutover with Rust launcher-side -p/--profile resolution for Rust-owned commands plus native `hermes profile`, `hermes profile list`, `hermes profile show`, and `hermes profile use` behavior matching Python for clean default/named profile homes, display paths, config model/provider rendering, skill counts, .env/SOUL status, table rendering, and sticky active-profile writes when no gateway is running. Remaining setup wizard, auth, provider/model selection, config migration commands, profile create/delete/alias/rename/export/import, live gateway-running detection in profile output, logs, update flow, skins/display mutation, skills commands, and interactive/TUI behavior remain Python-owned._ |  |  |  |  |
