...
| | _Rust native handler slice covers read_file line-window output, search_files content/file modes, write_file directory creation and byte counts, patch replace-mode result envelopes, post-patch content, protected write denial, todo replace/merge/read/injection semantics, and clarify validation/result shaping. Terminal/process, browser/web, delegate/subagent, MCP, memory/session, media, skills, clarify UI callbacks, cron/messaging/homeassistant, and kanban handlers remain documented deletion-blocking runtime boundaries in docs/rust-parity/tool-handler-boundaries.md with parity tests asserting every core tool is either native or explicitly covered before cutover._ |  |  |  |  |
...
| | _Started the full handler cutover gate by adding native Rust todo semantics, clarify validation/result shaping, and an executable coverage check over all 45 core tools from the Rust registry snapshot. Current state remains not deletion-safe; terminal/process/execute_code, browser/web, delegate/subagent, MCP, memory/session, media, skills, clarify UI callbacks, cron/messaging/homeassistant, and kanban are explicit deletion blockers with required Rust or external-service migration plans in docs/rust-parity/tool-handler-boundaries.md._ |  |  |  |  |
