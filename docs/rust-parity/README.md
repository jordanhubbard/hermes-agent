...
| | _Started with Rust-owned `hermes plugins list`, `hermes plugins enable`, and `hermes plugins disable`, matching Python for bundled/user manifest listing, enabled/disabled state rendering, missing-plugin errors, and plugins.enabled/plugins.disabled config writes. Per owner direction, Rust and Python plugin ABIs do not need to be mutually compatible unless an explicit RPC/IPC contract already exists; external user/pip Python plugins are not deletion blockers and can be converted to Rust on demand. In-repo Python plugins, plugin platform adapters, memory/image/context providers, dashboard plugin APIs, skills discovery/install/snapshot/slash injection, optional skills, and any existing RPC-backed extension contracts still need Rust ports or signed deferrals before Python source removal._ |  |  |  |  |
