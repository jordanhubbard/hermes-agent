...
| General plugins and plugin CLI commands | `hermes_cli/plugins.py`, `plugins/`, pip entry point group `hermes_agent.plugins` | `crates/hermes-integrations` owns plugin facade/hook/manifest/discovery contract snapshots. Actual plugins are dynamic Python modules and may register tools, hooks, platforms, skills, CLI commands, dashboard APIs, providers, and memory backends. | `python-primary`, `external-boundary candidate`, `deletion-blocker` | `hermes-fpr.8`, `hermes-fpr.5`, `hermes-fpr.6` | Compatibility suite for repo, user, and pip plugins; registered tools/hooks/platforms/CLI/dashboard routes; memory/image/context providers; and a documented Rust ABI or IPC migration path. |
...
| `crates/hermes-integrations` | Cron, batch, MCP, RL, and plugin boundary snapshots. | Replace or gate live integration runtimes and plugin compatibility. |
